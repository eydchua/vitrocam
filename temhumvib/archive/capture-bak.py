    ...
    while 1:
        ...
        if (GPIO.input(sensor)==True): #blotting pads are blotting
            ...
            if (GPIO.input(sensor)==False): #blotting is done 
                ...
                #take a video
                time.sleep(0.5)
                subprocess.run(["libcamera-vid -n -o vid.h264 -t 500 --vflip=1 --hflip=1 --framerate 210 --shutter 30 --width 640 --height 400 --awbgains 0,0 --flush 1 --denoise off --brightness 0 --contrast 1 --saturation 1 --sharpness 1 --gain 1 --save-pts timestamps.txt"], shell=True, capture_output=True)
                #turn off IR light
                GPIO.output(light1, False)
                GPIO.output(light2, False)
