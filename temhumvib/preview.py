...
def preview():
    ...
    subprocess.run(["libcamera-vid -n -o preview.h264 -t 20 --framerate 210 --vflip=1 --hflip=1 --shutter 20 --width 640 --height 400 --awbgains 0,0 --flush 1 --denoise off --brightness 0 --contrast 1 --saturation 0 --sharpness 1 --gain 1 --save-pts timestamps.txt"], shell=True, capture_output=True)
    #turn off the lights
    GPIO.output(light1,False)
    GPIO.output(light2,False)
    #extract video frames
    subprocess.run(["ffmpeg -i preview.h264 'preview_frame%03d.png'"], shell=True, capture_output=True)
    print("Preview captured! In %s" % path)
    GPIO.cleanup()
